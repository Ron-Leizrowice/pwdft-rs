...
def main():
    ...
    for section, in_file, a_ang, ecut_ry, k_grid, degauss_ry, nspin, pp_files, start_mag in SYSTEMS:
        ...
        summary_rows.append(
            (section, ecut_ry, k_grid, parsed["total_energy_ry"], e_ev,
             parsed["fermi_ev"], parsed["n_iter"], pp_files,
             parsed.get("total_magnetization_mub"), parsed.get("absolute_magnetization_mub"))
        )

    print("\n".join(lines))
    print()
    print("=== SUMMARY ===", file=sys.stderr)
    print(f"{'system':<22} {'ecut':>6} {'kgrid':<12} {'E_tot_Ry':>14} {'E_tot_eV':>14} {'E_F_eV':>8} {'it':>3} PP", file=sys.stderr)
    for row in summary_rows:
        section, ecut_ry, k_grid, e_ry, e_ev, ef, ni, pp_files, tm, am = row
        mag_str = f" |M|={am:.2f} μB" if am is not None else ""
        print(f"{section:<22} {ecut_ry:>6.1f} {str(k_grid):<12} {e_ry:>14.6f} {e_ev:>14.4f} {ef:>8.4f} {ni:>3} {','.join(pp_files)}{mag_str}", file=sys.stderr)

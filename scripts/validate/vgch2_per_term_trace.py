...
GaAs, NaCl, MgO. Reads from the checked-in ``qe_validation/<system>.out``
files; does NOT rerun QE.

Usage:
    uv run scripts/validate/vgch2_per_term_trace.py \\
...
# System -> QE output filename stem under qe_validation/.
...
def main() -> None:
    qe_dir = Path(__file__).resolve().parent.parent.parent / "qe_validation"
    ...
    for terms in all_terms:
        for term_name, ry_value in terms.values.items():
            if term_name == "fermi_eV":
                ...
                writer.writerow(
                    [terms.system, term_name, f"{ry_value:.8f}", f"{ev_value:.6f}"]
                )

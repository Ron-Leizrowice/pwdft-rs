...
This mirrors the already-landed `scripts/validate/beta_q_reference.py`
...
The resulting CSV at `scripts/validate/vgch_beta_l_heavy.csv` is then
...
def _extract_beta_projector(text: str, idx: int, mesh: int) -> tuple[int, np.ndarray]:
    ...
    if values.size != mesh:
        raise ValueError(
            f"{tag}: expected {mesh} values, got {values.size}"
        )
    return l, values
...
def main() -> int:
    ...
    pp_dir = repo_root / "pseudopotentials" / "nc" / "lda"
    out_csv = Path(__file__).resolve().parent / "vgch_beta_l_heavy.csv"

    rows = []
    hdr = (
        f"{'elem':>4} {'i':>3} {'l':>3} {'q (Bohr⁻¹)':>11} "
        f"{'F_Bohr (Bohr^3/2)':>22} {'F_Å (Å^3/2)':>22}"
    )
    print(hdr)
    print("-" * len(hdr))
    ...
    for elem in ELEMENTS:
        ...
        pp = parse_upf(upf)
        print(
            f"# {elem}: Z_val={pp['z_valence']:.2f}, n_proj={pp['n_proj']}, "
            f"ls={[l for (l, _) in pp['projectors']]}"
        )
        for i, (l, chi) in enumerate(pp["projectors"]):
            for q_bohr in Q_VALUES_BOHR:
                f_bohr = f_l_of_q_bohr(
                    pp["r_bohr"], pp["rab_bohr"], chi, l, q_bohr
                )
                # Convert q to 1/Å for the pwdft-convention integrand.
                q_ang = q_bohr / BOHR_TO_ANG
                f_ang = f_l_of_q_ang(
                    pp["r_bohr"], pp["rab_bohr"], chi, l, q_ang
                )
                rows.append((elem, i, l, q_bohr, q_ang, f_bohr, f_ang))
                print(
                    f"{elem:>4} {i:>3} {l:>3} {q_bohr:>11.6f} "
                    f"{f_bohr:>22.12e} {f_ang:>22.12e}"
                )

    with out_csv.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow([
            "element", "proj_index", "l",
            "q_bohr_inv", "q_per_ang",
            "F_bohr_3halves", "F_ang_3halves",
        ])
        for row in rows:
            w.writerow([
                row[0], row[1], row[2],
                f"{row[3]:.12e}", f"{row[4]:.12e}",
                f"{row[5]:.12e}", f"{row[6]:.12e}",
            ])

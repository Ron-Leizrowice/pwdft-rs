...
For the Cu FCC reference:
 - Reads `qe_validation/cu_fcc_scf.out`.
...
Usage:
    uv run scripts/validate/vgch2c_fermi_reference.py \\
        > scripts/validate/vgch2c_fermi_reference.csv
...
def parse_qe_output(path: Path, nspin: int = 1) -> QeBands:
    ...
    if len(eigenvalues) != nk:
        raise SystemExit(
            f"{path}: parsed {len(eigenvalues)} bands blocks, expected {nk}"
        )
    ...
    if len(nbnds) != 1:
        raise SystemExit(
            f"{path}: inconsistent nbnd across k-points: {sorted(nbnds)}"
        )
...
def find_fermi_pwdft(
    bands: QeBands, sigma_ev: float, occ_fn, spin_factor: float
) -> float:
...
def main() -> None:
    ...
    systems = [
        ("Cu_FCC", root / "qe_validation" / "cu_fcc_scf.out", 1),
        ...
        ("Fe_BCC_FM", root / "qe_validation" / "fe_bcc_fm_scf.out", 2),
        ("NaCl", root / "qe_validation" / "nacl_scf.out", 1),
        ("C_diamond", root / "qe_validation" / "c_diamond_scf.out", 1),
        ("MgO", root / "qe_validation" / "mgo_scf.out", 1),
    ]

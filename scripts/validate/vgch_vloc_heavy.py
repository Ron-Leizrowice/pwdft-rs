...
def v_local_g0_qe_units(
    r_bohr: np.ndarray, v_ry: np.ndarray, z_val: float, omega_bohr3: float
) -> float:
...
# ---------------------------------------------------------------------------
# Cells (match tests/qe_validation.rs).
# ---------------------------------------------------------------------------
CELLS = [
...
def main() -> int:
    ...
    out_csv = Path(__file__).resolve().parent / "vgch_vloc_heavy.csv"

    rows = []
    print(f"{'elem':>4}  {'Z_val':>5}  {'Ω (Bohr³)':>12}  "
          f"{'V(G=0) Ry':>14}  {'V(G=0) eV':>14}  {'N_atoms':>7}")
    print("-" * 70)
    for elem, bravais, a_ang, n_atoms_species, _n_atoms_prim in CELLS:
        ...
        v_g0_ry = v_local_g0_qe_units(
            pp["r_bohr"], pp["v_local_ry"], pp["z_valence"], omega_bohr3
        )
        v_g0_ev = v_g0_ry * RY_TO_EV
        rows.append((elem, pp["z_valence"], omega_bohr3, v_g0_ry, v_g0_ev, n_atoms_species))
        print(f"{elem:>4}  {pp['z_valence']:>5.1f}  {omega_bohr3:>12.4f}  "
              f"{v_g0_ry:>14.8f}  {v_g0_ev:>14.8f}  {n_atoms_species:>7}")

    with out_csv.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["element", "z_valence", "omega_bohr3",
                    "v_local_g0_ry", "v_local_g0_ev", "n_atoms_this_species"])
        for row in rows:
            w.writerow([row[0], row[1], f"{row[2]:.6f}",
                        f"{row[3]:.10e}", f"{row[4]:.10e}", row[5]])
    print(f"\nWrote {out_csv}")
    return 0

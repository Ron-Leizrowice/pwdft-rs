...
Also computes QE's own identity closure from `qe_validation/si_scf.out`
(one_electron + hartree + xc + ewald = internal_E) as the reference.

Usage
-----
    cd .claude/worktrees/<YOUR_WORKTREE>
    uv run scripts/validate/pcrs_residual_scan.py
...
SI_YAML_TEMPLATE = """\
# PCRS Si SCF scan; matches qe_validation/si_scf.in parameters.
...
PC_PAT = re.compile(
    r"E_(band|kinetic|local|nonlocal|hartree|xc|ewald)\s*=\s*([-+\d.Ee]+)"
)
E_SUM_PAT = re.compile(
    r"E_sum\(comp\)\s*=\s*([-+\d.Ee]+)\s+\(vs E_KS\s+([-+\d.Ee]+),\s*Δ=([-+\d.Ee]+)\)"
)
E_LOCAL_G0_PAT = re.compile(r"E_local\(G=0\)\s*=\s*([-+\d.Ee]+)")
ITERS_PAT = re.compile(r"SCF converged after (\d+) iterations")
FINAL_DRHO_PAT = re.compile(
    r"SCF iter\s+\d+:.*?Δρ=([-+\d.Ee]+)\s*$", flags=re.MULTILINE
)
...
def run_si_at_threshold(
    ...
    yaml_path.write_text(SI_YAML_TEMPLATE.format(
    ...
    print(f"  running [{tag}] conv_threshold={conv_threshold:.0e} ... ",
          end="", flush=True)
    try:
...
def qe_identity_closure(qe_out: Path) -> None:
    ...
    print(f"  one_electron   = {vals_ry['one_electron']:>16.10f} Ry  "
          f"({vals_ry['one_electron']*RY_TO_EV:>12.6f} eV)")
    print(f"  hartree        = {vals_ry['hartree']:>16.10f} Ry")
    print(f"  xc             = {vals_ry['xc']:>16.10f} Ry")
    print(f"  ewald          = {vals_ry['ewald']:>16.10f} Ry")
    print(f"  total (F)      = {vals_ry['total_energy']:>16.10f} Ry  "
          f"({vals_ry['total_energy']*RY_TO_EV:>12.6f} eV)")
    print(f"  -TS            = {vals_ry['smearing_ts']:>16.10f} Ry")
    print(f"  internal E     = {vals_ry['internal_E']:>16.10f} Ry  "
          f"(= F - (-TS))")
    s_ry = (vals_ry['one_electron'] + vals_ry['hartree']
            + vals_ry['xc'] + vals_ry['ewald'])
    ...
    if abs(diff_ev) < 1e-6:
        print(f"  => QE identity closes to < 1 ueV. Our ~1 eV residual is STRUCTURAL.")
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Root of the pwdft-rs repo / worktree"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-qe", action="store_true",
        help="Skip the QE identity-closure comparison"
    )
    parser.add_argument(
        "--keep-yaml", action="store_true",
        help="Keep tmp YAML configs after run"
    )
    parser.add_argument(
        "--max-iter", type=int, default=200,
        help="max_iter for pwdft-rs SCF at tightest thresholds"
    )
    ...
    if not args.no_qe:
        qe_si = repo_root / "qe_validation" / "si_scf.out"
        if qe_si.is_file():
        ...
        with tempfile.TemporaryDirectory(prefix="pcrs_scan_") as td:
            ...
            for ct in args.thresholds:
                res = run_si_at_threshold(
                    repo_root, tmpdir, ct,
                    max_iter=args.max_iter, sym_enabled=True,
                )
            ...
            for ct in args.thresholds:
                res = run_si_at_threshold(
                    repo_root, tmpdir, ct,
                    max_iter=args.max_iter, sym_enabled=False,
                )
                results_nosym.append(res)

            if args.keep_yaml:
                persist = repo_root / "scripts" / "validate" / "pcrs_yaml_cache"
            ...
            def print_table(label: str, results: list[dict]) -> None:
                ...
                print(f"  {'conv_thr':>10}  {'iters':>5}  {'final_Δρ':>10}  "
                ...
                for r in results:
                    ...
                    note = r.get("error", "")
                    print(f"  {ct:>10.0e}  {it:>5d}  {drho_str:>10}  "
                          f"{e_ks_s:>14}  {e_sum_s:>14}  {delta_s:>10}  {note}")
            ...
            if valid_sym:
                ...
                if ratio > 0.99:
                    print(f"  -> PLATEAU (residual is CONSTANT vs conv_threshold): structural, not SCF noise.")
                else:
            ...
            if valid_sym and valid_nosym:
                ...
                if d_sym > 100 * d_nosym:
                    print(f"  -> Turning symmetry OFF reduces residual by "
                          f"{d_sym/d_nosym:.1e}x; bug is in the SYMMETRY path.")
                    print(f"     Root cause hypothesis: density symmetrization on an FFT grid")
                    print(f"     incompatible with the fractional translations (Si Fd-3m has τ=1/4,")
                    print(f"     grid 18 is not divisible by 4). `check_grid_compatibility` only")
                    print(f"     validates rotations, not translations.")

...
def parse_charge_density(path: Path) -> dict:
    ...
    with open(path, "rb") as fp:
        ...
        if len(rec1) != 12:
            raise ValueError(
                f"rec 1 size {len(rec1)} != 12 (expected logical*4 + 2 int32)"
            )
        ...
        if len(rec3) != 3 * ngm_g * 4:
            raise ValueError(
                f"rec 3 size {len(rec3)} != 3*ngm_g*4 = {3 * ngm_g * 4}"
            )
        ...
        for ispin in range(nspin):
            ...
            if len(recn) != ngm_g * 16:
                raise ValueError(
                    f"rho rec {ispin + 1} size {len(recn)} != ngm_g*16 = {ngm_g * 16}"
                )
...
def _sanity_print(d: dict) -> None:
    ...
    print(f"mill range per-axis: ({d['mill'][:,0].min():+d}..{d['mill'][:,0].max():+d}) "

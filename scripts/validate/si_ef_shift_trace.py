...
`qe_validation/si_scf.out` reference, and emits a per-band shift CSV +
a short verdict on whether the shift is a rigid offset or per-band.

Usage (from the pwdft-rs worktree root):

    uv run scripts/validate/si_ef_shift_trace.py
...
- `qe_validation/si_scf.out` present at expected path.

Output:
- `scripts/validate/si_ef_shift.csv` with columns
...
QE_SI_OUT = REPO_ROOT / "qe_validation" / "si_scf.out"
VLOC_CSV = REPO_ROOT / "scripts" / "validate" / "vgch_vloc_heavy.csv"
OUTPUT_CSV = REPO_ROOT / "scripts" / "validate" / "si_ef_shift.csv"
...
def main() -> int:
    ...
    print(f"ΔE_F (pwdft − QE) = {e_f_pwdft - e_f_qe:+.4f} eV "

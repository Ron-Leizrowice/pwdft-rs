...
def si_fcc_g_shells(n_shells: int, a_bohr: float) -> list[tuple[int, float, float]]:
    ...
    return [(g2_int, tpba * math.sqrt(g2_int), (tpba**2) * g2_int) for g2_int in shells_int]
...
def main() -> int:
    ...
    out_csv = repo_root / "validation" / "reference" / "csv" / "vloc_g_si_reference.csv"
    ...
    print(f"  v_local_ry: [{pp['v_local_ry'][0]:.4e}, {pp['v_local_ry'][-1]:.4e}]")
    ...
    print()
    print(f"{'shell':>5}  {'|G|² (int)':>10}  {'|G| (Bohr⁻¹)':>14}  {'V_loc(G) (Ry)':>16}")
    print("-" * 56)
    for idx, (g2_int, g_bohr_inv, _g2_bohr2) in enumerate(shells):
        v_g = v_local_of_g_qe_units(pp["r_bohr"], pp["v_local_ry"], pp["z_valence"], g_bohr_inv, omega_bohr3)

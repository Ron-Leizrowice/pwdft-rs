...
**Most recent grooming pass — 2026-04-20 (GRM12, 5 PRs merged today, 70 cumulative).** Day split into two waves. Morning: URES landed (#172) + strict-stance revision (#173) rejecting bare `let _ =` in production. Afternoon: **ERR2 Phase 1 CLOSED** (#175 P1.d migrates the last ELEMENT + UPF sites; only P1.e `transplant.rs` 2-site mop-up remains); **DOCLEAN** (#176) fixes stale `SI_REFERENCE_FERMI_EV`, adds two CLAUDE.md Conventions gotchas, renames `assert_band_sum_matches_qe` → `assert_one_electron_sum_matches_qe` (RWHK H3). Three VGCH Class B/C diagnostics in parallel: **VGCH-2D** (#177) designs the Fe LDA PZ-vs-PW92 discriminator and surfaces a global LDA functional mismatch — every QE LDA deck reports `SLA+PW` (PW92) but pwdft-rs uses Slater+PZ-81 (`pwdft/pwdft-core/src/potential/xc.rs:143,394`); **VGCH-2E** (#178) reclassifies C diamond Class C → Class A light-atom (primary suspect NLCC ρ_core(G) on C); **VGCH-2F** (#179) refutes H-C4 — 4 new Ga/As/O/Cl ρ_core(G) pins all pass < 1e-5 e/Å³, Cu was already pinned by TRV2, so heavy-atom NLCC cannot explain the +8.87 eV Cu shared-density gap. H-C5 narrowed to a Cu two-radial-d-projector sum (filed as VNLM-CUD, Medium). Next-day entry points in priority order: (1) **PZPW** — rewire LDA to SLA+PW92; (2) **CNLC** — C NLCC ρ_core pin + NLCC-off ablation; (3) **VNLM-CUD** — Cu per-m two-radial-d diagnostic; (4) **ERR2-P1.e** — `transplant.rs` 2-site mop-up.

Older grooming-pass paragraphs rotated to `proposals/grooming-log.md`.

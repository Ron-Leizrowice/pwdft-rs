...
/// represents a type that can be used to slice a matrix, such as an index or a
/// range of indices
...
///
/// this implies that while each individual column is stored contiguously in
/// memory, the matrix as a whole may not necessarily be contiguous. the
/// implementation may add padding at the end of each column when overaligning
/// each column can provide a performance gain
///
...
///  2 │ 5 │ 8 │ 11
/// ```
/// the memory representation of the data held by such a matrix could look like
/// the following:
///
/// ```notcode
...
/// mutable view over a matrix, similar to a mutable reference to a 2d strided
/// [prim@slice]
///
/// # note
///
/// unlike a slice, the data pointed to by `MatMut<'_, T>` is allowed to be
/// partially or fully uninitialized under certain conditions. In this case,
/// care must be taken to not perform any operations that read the uninitialized
/// values, either directly or indirectly through any of the numerical library
/// routines, unless it is explicitly permitted
///
/// # move semantics
/// since `MatMut` mutably borrows data, it cannot be [`Copy`]. this means that
/// if we pass a `MatMut` to a function that takes it by value, or use a method
/// that consumes `self` like [`MatMut::transpose_mut`], this renders the
...
/// ```
/// the way to get around it is to use the [`reborrow::ReborrowMut`] trait,
...
pub mod generic {
	...
	impl<
		...
		Inner: for<'short> Reborrow<
				'short,
				Target = super::Ref<'short, T, Rows, Cols, RStride, CStride>,
			>,
	...
	impl<
		...
		Inner: for<'short> Reborrow<
			...
			> + for<'short> ReborrowMut<
				'short,
				Target = super::Mut<'short, T, Rows, Cols, RStride, CStride>,
			>,

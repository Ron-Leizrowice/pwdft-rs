...
def main() -> int:
    ...
    out_csv = repo_root / "validation" / "reference" / "csv" / "rho_core_g_reference.csv"
    ...
    for sys_cfg in systems:
        ...
        print(f"=== {name.upper()} (a = {sys_cfg['a_ang']} Å = {a_bohr:.4f} Bohr, Ω = {omega_bohr3:.4f} Bohr³) ===")
        print(f"  Q_core = ∫ 4πr²ρ_core(r) dr = {q_core:.6f} e")
        print(
            f"  {'shell':>5}  {'|G|² (int)':>10}  {'|G| (Bohr⁻¹)':>14}  "
            f"{'ρ_core(G) (e/Bohr³)':>22}  {'ρ_core(G) (e/Å³)':>20}"
        )
        print("-" * 84)

        for idx, (g2_int, g_bohr_inv) in enumerate(shells):
            rho_g_bohr = rho_core_of_g_qe_units(pp["r_bohr"], pp["rho_core_e_per_bohr3"], g_bohr_inv, omega_bohr3)
            rho_g_ang = rho_g_bohr / BOHR3_TO_ANG3
            rows.append((name, idx, g_bohr_inv, g2_int, rho_g_bohr, rho_g_ang))

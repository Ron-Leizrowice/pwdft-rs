...
def parse_upf_dij(path: Path) -> tuple[int, list[int], np.ndarray]:
    ...
    if dij_flat.size != n_proj * n_proj:
        raise ValueError(f"PP_DIJ: expected {n_proj * n_proj} values, got {dij_flat.size}")
...
def main() -> int:
    ...
    out_csv = repo_root / "validation" / "reference" / "csv" / "dij_si_reference.csv"
    ...
    if mismatches == 0:
        ...
        print(f"WARN: {mismatches} off-block entries have |D| > 1e-20; max |D_off_block| = {max_off_block:.3e} Ry")

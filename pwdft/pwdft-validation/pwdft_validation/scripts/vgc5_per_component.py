...
def qe_to_ev(data_ry: dict[str, float]) -> dict[str, float]:
    ...
    return {k: (v * RY_TO_EV if k != "fermi_ev" else v) for k, v in data_ry.items()}
...
def print_table(system: str, qe_ev: dict[str, float]) -> None:
    ...
    print(
        f"  [sum check]   sum(4 terms) = {s:.6f} eV, internal_E = "
...
def main() -> int:
    from pwdft_validation import CSV_REF_DIR, QE_REF_DIR

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--qe-dir", type=Path, default=QE_REF_DIR, help="Directory containing si_scf.out and fe_bcc_fm_scf.out"
    )
    parser.add_argument("--out-dir", type=Path, default=CSV_REF_DIR, help="Directory to write vgc5_qe_*.csv")

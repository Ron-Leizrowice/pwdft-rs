...
//! - `gpu_fresh_<n>` — GPU kernel with per-call `create_buffer` / `create_bind_group` (the fallback
//!   when `prepare_buffers` has not been invoked). Serves as the "before" baseline for GOPT PR-B.
//! - `gpu_pooled_<n>` — GPU kernel with the `BufferPool` warm: persistent storage buffers, cached
//!   bind groups, uniform updates via `write_buffer`. This is the production path the SCF driver
//!   uses.
//!
...
use pwdft_core::{gpu::GpuAccelerator, potential::xc};
...
fn make_complex_data(n: usize, seed: f64) -> Vec<Complex64> {
    (0..n)
        .map(|i| Complex64::new((i as f64 * seed).sin() * 0.01, (i as f64 * seed * 1.3).cos() * 0.01))
        .collect()
}
...
fn bench_hartree(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        group.bench_function(format!("gpu_fresh_{n}"), |b| {
            b.iter(|| {
                black_box(gpu_fresh.hartree_potential(black_box(&rho_g), black_box(&g_squared), fourpi_e2));
            });
        });

        // Pooled path: warm up a separate accelerator so the pool is sized to n.
        let mut gpu_pooled = GpuAccelerator::try_new().expect("second GPU init succeeded during first");
        gpu_pooled.prepare_buffers(n, &g_squared);
        group.bench_function(format!("gpu_pooled_{n}"), |b| {
            b.iter(|| {
                black_box(gpu_pooled.hartree_potential(black_box(&rho_g), black_box(&g_squared), fourpi_e2));
            });
        });
    }
    group.finish();
...
fn bench_v_eff(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        group.bench_function(format!("gpu_fresh_{n}"), |b| {
            b.iter(|| {
                black_box(gpu_fresh.v_eff_assembly(black_box(&v_local), black_box(&v_h), black_box(&v_xc)));
            });
        });
        ...
        group.bench_function(format!("gpu_pooled_{n}"), |b| {
            b.iter(|| {
                black_box(gpu_pooled.v_eff_assembly(black_box(&v_local), black_box(&v_h), black_box(&v_xc)));
            });
        });
    }
    group.finish();
...
fn bench_lda_xc(c: &mut Criterion) {
    ...
    for &n in SIZES {
        let rho_r: Vec<f64> = (0..n).map(|i| 0.01 + (i as f64 / n as f64) * 0.5).collect();
        let g_squared = make_g_squared(n);

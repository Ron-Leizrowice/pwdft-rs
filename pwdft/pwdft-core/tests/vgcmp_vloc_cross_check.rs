...
//!
//! This test consumes `validation/src/pwdft_validation/scripts/vloc_g_si_reference.csv`, generated
//! by `validation/src/pwdft_validation/scripts/vloc_g_reference.py`, which computes V_local(G) for
...
const CSV_REL_PATH: &str = "validation/reference/csv/vloc_g_si_reference.csv";
    ...
    let Some(rows) = load_reference_csv(&csv_path) else {
        eprintln!(
            "VGCMP: reference CSV not found at {} — skipping cross-check.\n\
             (Run `uv run validation/src/pwdft_validation/scripts/vloc_g_reference.py` to regenerate.)",
            csv_path.display()
        );
        return;
    };

...
//!
//! Compiles and consumes `data/csv/beta_q_si_reference.csv`, generated
//! by `pwdft-validate reference beta`. The reference computes, for each
...
//!
//! In pwdft-core the same integral is computed by the private helper
...
//! - pwdft-core: Å^(3/2), q in Å⁻¹ (internal).
    ...
    let Some(rows) = load_reference_csv(&csv_path) else {
        eprintln!(
            "VGCMP Phase 2: reference CSV not found at {} — skipping cross-check.\n\
             (Run `uv run pwdft-validate reference beta` to regenerate.)",
            csv_path.display()
        );
        return;
    };

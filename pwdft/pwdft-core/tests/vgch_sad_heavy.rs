...
//! (`validation/src/pwdft_validation/scripts/vgch_sad_heavy.py`) for every VGCH-class system.
//!
...
const CSV_REL_PATH: &str = "validation/reference/csv/vgch_sad_heavy.csv";
const SAMPLES_CSV_REL_PATH: &str = "validation/reference/csv/vgch_sad_heavy_samples.csv";
...
#[test]
#[ignore = "TSPL Tier-2: VGCH Phase 1c SAD diagnostic — needs CSV from validation/src/pwdft_validation/scripts/vgch_sad_heavy.py. Run with `-- --ignored`."]
fn test_vgch_sad_matches_qe_convention_reference() {
    ...
    let Some(ref_rows) = load_reference_csv(&csv_path) else {
        panic!(
            "reference CSV not found at {}: regenerate via `uv run validation/src/pwdft_validation/scripts/vgch_sad_heavy.py`",
            csv_path.display()
        );
    };
    ...
    assert!(
        ...
        "VGCH Phase 1c: raw-sample ρ(r) diff exceeded {TOL_RAW_SAMPLE_RHO:.0e} on one or more \
         ...
         QE-convention Python reference. See table above and `validation/src/pwdft_validation/scripts/vgch_sad_heavy.py`."
    );

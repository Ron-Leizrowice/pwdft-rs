//! Validate pwdft-core SCF results against Quantum ESPRESSO 7.5 reference data
...
//! the measured pwdft-core and QE values. Drop an `#[ignore]` once the
//! attributed residual category closes.
//!
//! [`KGridShift`]: pwdft_core::kpoints::KGridShift
...
/// Parameters for a QE-vs-pwdft-core comparison.
///
...
/// against its pwdft-core equivalent
...
/// residual was artificially small because pwdft-core's pre-TSEN
...
/// (≈ 1.35 eV) that the total energy did not: pwdft-core set V_eff(G=0) = 0
...
/// on the pwdft-core side — a pwdft-core-only ecut sweep at 8×8×8 against
...
/// converged basis:
///
/// | grid   | pwdft-core (eV) | QE (eV)   | \|ΔE\| (meV) |
/// |--------|---------------|-----------|--------------|
...
///   pwdft-core post-TSEN (Γ-centered): E = -3049.016 eV (8×8×8, 15 Ry, Kerker)
...
///
/// Reference values (post-TSEN):
///   pwdft-core: E = −1992.535 eV
///   QE:       E = −2003.241 eV  (−147.235_477_68 Ry)
///   residual: ~10.71 eV
///   (MgO is a wide-gap insulator; QE's −TS is sub-meV and pwdft-core's
...
///   pwdft-core E_xc = −392.5675 eV   (pin in `tests/vgc5_per_component_si.rs`)
...
fn test_fe_bcc_ewald_vs_qe() {
    ...
    let e_ewald = pwdft_core::ewald::ewald_energy(&crystal, &[&pp_fe]);
...
///
/// Post-GGAP-Phase-A.1 pwdft-core converges to `E_total ≈ -230.0772 eV`

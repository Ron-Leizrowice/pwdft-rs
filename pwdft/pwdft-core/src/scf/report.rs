...
pub(super) fn log_iteration(pb: &ProgressBar, r: &IterationReport) {
    ...
    let de_str = r.de.map_or("N/A".to_string(), |de| format!("{de:.2e}"));
    match &r.spin {
        None => {
            pb.set_message(format!("E={:.4} eV  Δρ={:.1e}", r.e_total, r.delta));
...
pub(super) fn log_convergence_summary(e_total: f64, e_harris: f64, hf_diff: f64, free_energy: f64, energy_sigma0: f64) {
...
pub(super) fn log_components(c: &EnergyComponents, e_total: f64) {
    ...
    info!("  E_local      = {:.6}  (includes V_loc(G=0)·N_el)", c.e_local);

...
// `tests/` see the `pwdft_core` lib compiled without `cfg(test)`, so gated
...
fn pbe_exchange(rho: f64, grad_rho_mag: f64) -> (f64, f64, f64) {
    ...
    // Unit conversion back to pwdft-core native units (eV, Å).
...
fn pbe_correlation(rho: f64, grad_rho_mag: f64) -> (f64, f64, f64) {
    ...
    // Convert to pwdft-core native units (eV, Å) — same scheme as `pbe_exchange`.
    let bohr5 = bohr4 * bohr;
...
fn pbe_correlation_spin(
    ...
    // Convert to pwdft-core native units (eV, Å).
    let bohr5 = bohr4 * bohr;
    let eps_c = eps_c_ha * ha / bohr3;

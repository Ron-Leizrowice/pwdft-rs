...
**Profile split: local vs. CI.** Local `cargo test` uses the O3 `[profile.test]` from `Cargo.toml` (per TPRF) — warm-cache M3 Max is runtime-bound, and `opt-level=3` keeps the few hot-path unit tests fast (12 s warm Tier-1). CI is the other axis: Ubuntu runners are compile-bound on the test-binary link path, so `.github/workflows/rust.yml` overrides Tier-1 to `cargo test --profile=dev -p pwdft-core` (per TDBG). Pre-TDBG baseline on main run `24663826105` was 343 s for the test step, 643 s (10m43s) total job wall. Post-TDBG warm-cache steady state is 281 s test step, 352 s (5m52s) total job wall — ~18% off the test step alone, ~45% off the full job (the shared `pwdft-ci-tier1-dev` cache key lets the `cargo test` step reuse clippy's debug-profile dep artifacts). Guardrail ceiling in the workflow is 330 s — a regression past the pre-TDBG baseline still fails the step. Tier-2 stays on the O3 profile everywhere — those SCF-loop tests would be glacial at `opt-level=0`.

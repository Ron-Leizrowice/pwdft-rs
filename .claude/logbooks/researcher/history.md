...
**Load-bearing cross-language convention (→ candidate CLAUDE.md promotion):** Rust's `f64::round()` is half-away-from-zero; numpy's `np.round` is half-to-even (banker's rounding). At exactly `d = ±0.5` they disagree in sign of the integer. Any Rust↔Python validation that maps real-valued distances/fractions to grid bins must either (a) avoid `.round()` entirely or (b) use an explicit wrap like `d - (d + 0.5).floor()` on both sides. No production-code impact yet; diagnostic-only. (Promoted to CLAUDE.md § Conventions via DOCLEAN #176.)

Pre-2026-04-19 entries (PCRS, VGC5, VGCMP Phases 1–4, SYKP, QEVL+SPXC, KBTF handoff) archived to `.claude/logbooks/archive/researcher-2026-04.md`.

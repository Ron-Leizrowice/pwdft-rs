...
//! (`scripts/validate/vgch_sad_heavy.py`) for every VGCH-class system.
//!
//! The hypothesis under test: does pwdft-rs' SAD (its PP_RHOATOM Bessel
...
//!   ρ(r) = Σ_G ρ(G) exp(+iG·r)                (pwdft-rs unnormalized IFFT)
...
//! regime is the negative-clamp step in pwdft-rs (QE skips it; see
...
const CSV_REL_PATH: &str = "scripts/validate/vgch_sad_heavy.csv";
const SAMPLES_CSV_REL_PATH: &str = "scripts/validate/vgch_sad_heavy_samples.csv";
...
///     core, where pwdft-rs clamps O(1e-5 e) of negative Gibbs ringing
...
struct SystemDiagnosis {
    ...
    stats: pwdft_rs::scf::initial_density::SadDiagnosticStats,
    n_electrons: f64,
}

/// Compute pwdft-rs SAD on the fixed grid and return shell-averaged
...
fn diagnose_system(
    ...
    // ecutwfc in eV (pwdft-rs internal)
...
#[test]
#[ignore = "TSPL Tier-2: VGCH Phase 1c SAD diagnostic — needs CSV from scripts/validate/vgch_sad_heavy.py. Run with `-- --ignored`."]
fn test_vgch_sad_matches_qe_convention_reference() {
    ...
    let Some(ref_rows) = load_reference_csv(&csv_path) else {
        panic!(
            "reference CSV not found at {}: regenerate via `uv run scripts/validate/vgch_sad_heavy.py`",
            csv_path.display()
        );
    };
    ...
    // where it should. ----
    let samples_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(SAMPLES_CSV_REL_PATH);
    ...
    for sys in &systems() {
        ...
        let (_dims, _pre, rho_final, _stats) = pwdft_rs::scf::initial_density::build_sad_density_for_diagnostic_verbose(
            &diag.crystal,
            &{
    ...
    assert!(
        ...
        "VGCH Phase 1c: raw-sample ρ(r) diff exceeded {TOL_RAW_SAMPLE_RHO:.0e} on one or more \
         ...
         QE-convention Python reference. See table above and `scripts/validate/vgch_sad_heavy.py`."
    );
    ...
    println!("{}", "-".repeat(72));
    let mut system_stats: HashMap<String, pwdft_rs::scf::initial_density::SadDiagnosticStats> =
        HashMap::new();
    for sys in &systems() {
        let diag = diagnose_system(sys, &pp_cache);

...
//!
//! Seeds pwdft-rs' SCF with QE's converged Cu FCC density and runs
...
//! ~1 meV/term, the H3 mixer-basin hypothesis is confirmed: pwdft-rs and
...
//!    `qe_validation/cu_fcc_scf.in` has `disk_io='low'` and does NOT
...
//!    `scripts/validate/vgch2_parse_qe_density.py --verbose` → produces
...
//!    the Miller-indexed density onto pwdft-rs' FFT grid, converting
...
/// Map (mill_index, rho_g_qe) pairs onto pwdft-rs' FFT grid.
///
...
/// ρ(G=0) under pwdft-rs' `1/N`-forward convention is `⟨ρ⟩ = N_el / Ω`, so
...
// ---------------------------------------------------------------------------
// QE reference (qe_validation/cu_fcc_scf.out, 8×8×8 Γ-centered, ecut=25 Ry).
// ---------------------------------------------------------------------------
...
fn cu_bin_path() -> Option<PathBuf> {
    ...
    let candidates = [
        ...
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("qe_validation/cu_rho_qe.bin"),
    ];
    candidates.into_iter().find(|p| p.is_file())
}

#[test]
#[ignore = "TSPL Tier-2: VGCH-2B transplant diagnostic — seed Cu FCC SCF from QE density, compare iter-1 per-term energies; regenerate `cu_rho_qe.npz` via scripts/validate/vgch2_parse_qe_density.py"]
fn test_cu_fcc_transplant_iter1() {
    let bin_path = cu_bin_path().unwrap_or_else(|| {
        panic!(
            "VGCH-2B: expected QE density at /tmp/vgch2b_cu/cu_rho_qe.bin or \
             qe_validation/cu_rho_qe.bin — regenerate via \
    ...
    let params = ScfParams {
        ...
        // Force the FFT grid to match QE's 15×15×15 (see qe_validation/cu_fcc_scf.out
        // "Dense grid: 1363 G-vectors FFT dimensions: (15, 15, 15)"). Without
        // this override, pwdft-rs picks a 2/3/5-smooth grid that may differ,
    ...
    // Map QE's rho_g (e/Bohr³) onto pwdft-rs' FFT grid (e/Å³).

//! VGCMP Phase 1 — cross-check pwdft-rs V_local(G) against an independent
//! Python reference for Si (ONCVPSP LDA).
//!
//! This test consumes `scripts/validate/vloc_g_si_reference.csv`, generated
//! by `scripts/validate/vloc_g_reference.py`, which computes V_local(G) for
...
//!
//! The reference is in QE native units (Ry, Bohr). pwdft-rs's
...
const CSV_REL_PATH: &str = "scripts/validate/vloc_g_si_reference.csv";
    ...
    let Some(rows) = load_reference_csv(&csv_path) else {
        eprintln!(
            "VGCMP: reference CSV not found at {} — skipping cross-check.\n\
             (Run `uv run scripts/validate/vloc_g_reference.py` to regenerate.)",
            csv_path.display()
        );
        return;
    };

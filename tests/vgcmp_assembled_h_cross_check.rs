//! VGCMP Phase 4 — cross-check pwdft-rs' assembled Hamiltonian diagonal
...
//! `scripts/validate/vgcmp_phase4_reference.csv` which was produced by an
...
//! Units
//! -----
//! CSV reference values are in Ry (QE native). pwdft-rs Hamiltonian entries
...
const CSV_REL_PATH: &str = "scripts/validate/vgcmp_phase4_reference.csv";
...
fn vgcmp_phase4_hbar2_over_2m_consistency() {
    // Pre-flight check: the SI-derived HBAR2_OVER_2M in pwdft-rs (eV·Å²) must
    ...
    eprintln!("  pwdft-rs HBAR2_OVER_2M (SI-derived)     = {HBAR2_OVER_2M:.10} eV·Å²");
    ...
    let Some(rows) = load_reference_csv(&csv_path) else {
        eprintln!(
            "VGCMP Phase 4: reference CSV not found at {} — skipping cross-check.\n\
             (Run `uv run scripts/validate/vgcmp_phase4_assembled_h.py` to regenerate.)",
            csv_path.display()
        );
        return;
    };

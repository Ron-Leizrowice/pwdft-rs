...
//! decomposition — pwdft-rs and QE converge to different self-consistent
...
//! density ρ_QE(G) into pwdft-rs, running **exactly one** SCF iteration
...
//!   residual at pwdft-rs self-consistency is then mixer-basin (H3).
...
//! pwdft-rs's engineering surface. The routine mirrors the first
...
/// FFT grid with the pwdft-rs ordering (see `scf::grid::g_vector_at_dims`).
...
/// and the same convention pwdft-rs' `density_r_to_g` applies. Callers
...
pub fn run_scf_iter1_from_rho_g_fft(
    ...
    // Materialise ρ_in on the real-space grid. pwdft-rs' inverse FFT is
    ...
    // with rho_r under the pwdft-rs 1/N convention. The caller's

...
///
/// For small |x|: Taylor series for erf(x), then 1 - erf.
/// For |x| >= 0.5: rational Chebyshev approximation (Numerical Recipes §6.2).
/// Accuracy: ~1e-14 relative error across the full range.
fn erfc(x: f64) -> f64 {
    let ax = x.abs();
    if ax > 27.0 {
    ...
    let result = if ax < 0.5 {
        // Small x: erf(x) via Taylor series
        1.0 - erf_series(ax)
    } else {
        // Chebyshev approximation for erfc (Numerical Recipes eq. 6.2.8)
        let t = 2.0 / (2.0 + ax);
        let ty = 4.0 * t - 2.0;
        ...
        t * (-ax * ax + 0.5 * (c[0] + ty * d) - dd).exp()
    };

    if x < 0.0 { 2.0 - result } else { result }
}

/// erf(x) via Taylor series — accurate for |x| < 0.5.
